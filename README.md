...
DSH（DeepSeek Harness）Web 插件：Agent / 子代理 / 工作流任务完成时，以及 **agent 停下来等你操作时**（需要审批、向你提问），弹出真正的 **Windows 系统通知**（带提示音与 DeepSeek 图标），可选 PushPlus 微信推送。浏览器关闭也能收到。
...
- **询问用户**：agent 调用 `ask_user_question` 主动停下咨询时，通知「Agent 正在问你」（含问题文本），计划评审（plan-review）同样覆盖；
- **分类开关**：主回合 / 子代理 / 工作流 / 需要审批 / 询问用户 独立开关，另有总开关一键关闭全部通知；
...
- 毛玻璃配置面板：总开关 / 提醒对象（主回合、子代理、工作流、需要审批、询问用户）/ 手机推送，含保存、测试推送、测试通知。
...
4. 「提醒对象」中按需开关：**需要审批**（agent 提权/越权审批，仅审批策略 `ask` 时提醒）、**询问用户**（agent 提问 / 计划评审）。
